...
#src_dir = dirs['src']['yahoo']
#rss_dir = dirs['rss_dir']


src_dir = info['src_dir']['yahoo']
...
class Inware:
    ...
    def filter(self, src):
        pre = str(BeautifulSoup(src, "html.parser").findAll('div',{'itemprop': 'articleBody'}))
        post = BeautifulSoup(pre, "html.parser").findAll('p')
        return post
    ...
    def eval_json(self):
        ...
        with open('crawler.json', 'r', encoding='utf-8') as json_file:
            self.ch_refs = list(json.load(json_file)['yahoo']['channels'].values())
        os.chdir(src_dir)
    ...
    def channelDate(self):
        fp = init_fp('http://tw.news.yahoo.com/rss/society').feed.published_parsed
