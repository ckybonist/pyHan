...
#src_dir = dirs['src']['udn']
#rss_dir = dirs['rss_dir']


src_dir = info['src_dir']['udn']
...
class Inware:
    ...
    def filter(self, src):
        pre = str(BeautifulSoup(src, "html.parser").findAll('td',{'valign' : 'top'}))
        mid = str(BeautifulSoup(str(pre), "html.parser").findAll('div', {'class' : 'story'}))
        post = BeautifulSoup(str(mid),"html.parser").find('p')
        return post
    ...
    def eval_json(self):
        ...
        with open('crawler.json', 'r', encoding='utf-8') as json_file:
            self.ch_refs = list(json.load(json_file)['udn']['channels'].values())
        os.chdir(src_dir)
    ...
    def channelDate(self):
        fp = init_fp('http://udn.com/udnrss/focus.xml').feed.published_parsed
...
if __name__ == "__main__":
    ...
    # define dir name of corpora-sources
